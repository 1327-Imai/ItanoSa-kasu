...
namespace Myfunc {
	...
	namespace Affine {
		...
		//ワールド変換行列を生成する関数
		Matrix4 CreateMatScale(Matrix4 affineMat , Vector3 scale);				//スケーリング行列を生成する関数
		Matrix4 CreateMatRotation(Matrix4 affineMat , Vector3 rotation);			//回転行列を生成する関数
		Matrix4 CreateMatTranslation(Matrix4 affineMat , Vector3 translation);	//平行移動行列の生成をする関数
	}
